...
const DEAL_COLUMNS = "id,title,image,price,mrp,discount_percentage,source,standard_link,affiliate_link,metadata,asin";
...
export async function applyUpdate(deal: DealRow, live: NormalizedProduct): Promise<ApplyResult> {
  ...
  if (live.currentPrice == null || live.currentPrice <= 0) {
    await supabaseAdmin.from("deals").update({ last_checked_at: new Date().toISOString(), asin: live.asin }).eq("id", deal.id);
  ...
  console.log(`[amazon] deal ${deal.id} asin=${live.asin} price ${deal.price} -> ${newPrice} (mrp ${newMrp}, changed=${priceChanged})`);

  if (priceChanged) {
    const { error: histError } = await supabaseAdmin.from("deal_price_history").insert({ deal_id: deal.id, price: newPrice, mrp: newMrp });
...
/** Persist ASINs that can already be derived from the existing Amazon URLs. */
async function backfillAsins(deals: DealRow[]): Promise<DealRow[]> {
  for (const deal of deals) {
    if (deal.asin) continue;
    const asin = asinFor(deal);
    if (!asin) continue;
    const { error } = await supabaseAdmin.from("deals").update({ asin }).eq("id", deal.id);
    if (error) {
      console.error(`[amazon] ASIN backfill failed for deal ${deal.id}: ${error.message}`);
      continue;
    }
    deal.asin = asin;
    console.log(`[amazon] backfilled ASIN ${asin} for deal ${deal.id}`);
  }
  return deals;
}
...
export async function hydrateDeal(dealId: string): Promise<ApplyResult> {
  const { data, error } = await supabaseAdmin.from("deals").select(DEAL_COLUMNS).eq("id", dealId).maybeSingle();
  ...
  if (!live) return { dealId, asin, status: "skipped", reason: "no Creators API result" };
...
export async function refreshBatch(limit = 30): Promise<{
  ...
  const { data, error } = await supabaseAdmin.from("deals").select(DEAL_COLUMNS).eq("is_active", true).order("last_checked_at", { ascending: true, nullsFirst: true }).limit(Math.min(Math.max(limit, 1), 100));
  if (error) throw new Error(`deal batch lookup failed: ${error.message}`);

  const deals = await backfillAsins((data ?? []) as unknown as DealRow[]);
  const withAsin = deals.map((d) => ({ deal: d, asin: asinFor(d) })).filter((x): x is { deal: DealRow; asin: string } => Boolean(x.asin));
  const withoutAsin = deals.filter((d) => !asinFor(d));
  for (const d of withoutAsin) console.warn(`[amazon] deal ${d.id} skipped: no valid ASIN (source=${d.source ?? "unknown"})`);
  console.log(`[amazon] refresh selected ${deals.length} deal(s); ${withAsin.length} with ASIN, ${withoutAsin.length} skipped`);

  if (withAsin.length === 0) return { checked: 0, updated: 0, skipped: 0, withoutAsin: withoutAsin.length, results: [] };
  ...
  for (const { deal, asin } of withAsin) {
    ...
    if (!live) {
      results.push({ dealId: deal.id, asin, status: "skipped", reason: "no Creators API result" });
      continue;
    }
    try {
  ...
  return { checked: results.length, updated, skipped, withoutAsin: withoutAsin.length, results: [...results, ...withoutAsin.map((d) => ({ dealId: d.id, asin: null, status: "skipped" as const, reason: "no ASIN" }))] };
}
